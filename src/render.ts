...
const registerRenderEvents = (scene: Scene, events: Events) => {
                ...
                const prepareFrame = async (frameTime: number, skipSort = false): Promise<Splat | null> => {
                    ...
                    if (hasCamKeys) {
                        ...
                        if (val && val.length >= 7) {
                            ...
                            // Set tween targets via setPose (needed for 360 mode
                            // which reads scene.camera.azim for face rotation)
                            scene.camera.fov = val[6];
                            scene.camera.setPose(pos, rawTgt, 0);

                            // For non-360 mode, use poseOverride to bypass the
                            // orbit camera tween system entirely. This ensures
                            // the camera position/rotation exactly matches the
                            // spline keyframe values without any orbit camera
                            // conversion roundtrip or controller interference.
                            if (!is360) {
                                // Compute rotation via Entity.lookAt
                                const camEntity = scene.camera.mainCamera;
                                camEntity.setLocalPosition(pos.x, pos.y, pos.z);
                                camEntity.lookAt(rawTgt.x, rawTgt.y, rawTgt.z);
                                const rotation = camEntity.getLocalRotation();

                                // Compute near/far clipping planes from scene bounds
                                const fwd = new Vec3().sub2(rawTgt, pos);
                                fwd.normalize();
                                const bound = scene.bound;
                                const boundRadius = bound.halfExtents.length();
                                const centerDist = new Vec3().sub2(bound.center, pos).dot(fwd);
    ...
    // ----------------------------------------------------------------
    events.function('render.turntableVideo', (settings: { frameRate: number; width: number; height: number; bitrate: number; format: string; codec: string }, fileStream: FileSystemWritableFileStream) => {
        const renderImpl = async () => {
            ...
            try {
                const { frameRate, width, height, bitrate, format, codec: codecChoice } = settings;
                ...
                // Save focal point — in 'look' (fly) mode the focal point may
                // have been shifted by adjustHeading. Reset it to the scene
                // center so the turntable orbits around the scene, not around
                // an offset point. Also clear any frozen lookCameraPos.
                savedFocalPoint = scene.camera.focalPoint.clone();
                const sceneCenter = scene.bound.center.clone();
                scene.camera.lookCameraPos = null;
                scene.camera.setFocalPoint(sceneCenter, 0);
                scene.camera.onUpdate(0);
                ...
                const encodeFrame = async (frameIndex: number) => {
                    // Rotate camera: distribute 360° evenly across all frames
                ...
                const filename = () => {
                    ...
                    return `${sanitizeFilename(base)}_turntable.${fileExtension}`;
                };
        ...
        if (navigator.locks) {
            return navigator.locks.request('splatroom-turntable-video-render', renderImpl);
        }
        return renderImpl();
    });
};
