...
class Menu extends Container {
    constructor(events: Events, args = {}) {
        ...
        const fileMenuPanel = new MenuPanel([{
            ...
            text: () => 'Save Color Grade (.sscg)',
            icon: createSvg(sceneSave),
            isEnabled: () => events.invoke('grade.canSave'),
            onSelect: () => events.fire('grade.save')
        }, {
            text: () => 'Load Color Grade (.sscg)',
            icon: createSvg(sceneOpen),
            isEnabled: () => events.invoke('grade.canLoad'),
            onSelect: () => events.fire('grade.load')
        }, {
        ...
        const renderMenuPanel = new MenuPanel([{
            ...
            text: () => i18n.t('menu.render.turntable', { ellipsis: true }),
            icon: createSvg(sceneExport),
            onSelect: async () => await events.invoke('show.turntableVideoDialog')
        }]);

        const helpMenuPanel = new MenuPanel([{
            text: () => i18n.t('menu.help.user-guide'),
            icon: 'E232',
            onSelect: () => window.open('https://github.com/ByronHuangsir/SplatRoom', '_blank')?.focus()
        }, {
            ...
            icon: 'E336',
            onSelect: () => window.open('mailto:1924705842@qq.com', '_blank')?.focus()
        }, {
            // separator
        }, {
